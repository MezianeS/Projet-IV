<?php 
	class Users extends Controller { //view est l'extension du controller.
			...
			public function register(){
				...
				if($_SERVER['REQUEST_METHOD'] == 'POST' ){
					...
					// Validate Name
					if(empty($data['name'])){
						$data['name_err'] = 'Entrez votre nom';
					}
					...
					if(empty($data['password'])){
						...
						$data['password_err'] = 'Au minimum 6 caractères';
					}

					//Valider le confirm password 
					...
					if($data['password'] != $data['confirm_password']){
  					...
  					if(empty($data['email_err']) && empty($data['name_err']) && empty($data['password_err']) && empty($data['confirm_password_err'])){
  						...
  						if($this->userModel->register($data)){
  							flash('register_success', 'Inscription valide, connectez-vous');
					...
					//Make sure errors are empty
  					if(empty($data['email_err']) && empty($data['password_err'])){
  						...
  						if($loggedInUser){
  							...
  							$data['password_err'] = 'Le mot de passe est incorrecte';
